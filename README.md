...
- `verify-coverage.sh <manifest> <report>` — set-diff against the approved, frozen checklist manifest: fails if any approved item is missing a result row or carries anything other than a **terminal bucket** (`pass`/`fail`/`blocked`/`flaky`/`N/A` — a whitelist, so a disguised skip like `skipped`/`deferred` can't pass as done), or any item isn't rooted in a defined user journey (a code-built checklist).
