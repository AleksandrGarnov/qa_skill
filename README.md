...
```
branch → context (Jira AC) → review + research + adversarial break-it → checklist
...
| Skill | What it does |
|-------|------------|
| `test-iteration` | Orchestrates a full pre-merge QA cycle for a branch: context from CLAUDE.md (incl. the test-docs path) **+ Jira acceptance criteria** + **prior-test check** (if the task was tested before, start from the old bug doc) → deterministic diff → `branch-review` + `qa-research` (in parallel) → **adversarial break-it pass** (a subagent whose sole goal is to break the service; every landed attack becomes a checklist item with a reproducing test, nothing fixed) → triage anchored on the ACs (with code-derived checks + **combinatorial input design** (equivalence partitioning + boundary value analysis + pairwise for multi-parameter features) + blast-radius + **domain packs** + hotspots) → right-sized checklist (with per-item test-data preconditions) + measurable exit criteria (locked mandatory core) → **independent completeness review** → approval → entry-checked staging run (every result tagged with its **evidence type**) → report with two-axis traceability (AC↔tests and changed-code↔tests), an **evidence-gate** (a runtime AC can't pass on code-read or a mocked unit test — a clean GO is blocked until every AC/critical-path pass quotes a raw observation **checked against an independently-derived expected**, not the implementation's own output; oracle-hard cases fall back to a metamorphic invariant), and a fail-closed GO / GO-with-deferrals / NO-GO verdict → re-test loop with round tracking. |
...
- `verify-report.sh <report.md>` — structural completeness gate for a filled report: fails closed if any item's execution record (`how run` / result / evidence / raw output) or the `Prior-test basis` line is blank or a placeholder. Run before the verdict. Checks structure, not truthfulness. **Section/label anchors are EN + RU by default and env-configurable** (`QA_RE_RESULTS`, `QA_RE_PRIOR`, `QA_RE_VERDICT`, …) so a localized report (e.g. `## Результаты по чек-листу` / `**Вердикт:**`) isn't false-blocked.
- `verify-coverage.sh <manifest> <report>` — set-diff against the approved, frozen checklist manifest: fails if any approved item is missing a result row or carries anything other than a **terminal bucket** (`pass`/`fail`/`blocked`/`flaky`/`N/A` — a whitelist, so a disguised skip like `skipped`/`deferred` can't pass as done), any item isn't rooted in a defined user journey (a code-built checklist), or any item's **`Expected source` is empty or names the implementation itself** (the oracle rule — a green observation checked against the code's own output proves nothing).
- `verify-context.sh <manifest>` — the **system input-guidelines gate**: fails closed unless the manifest's `## Context` carries the *fetched* PR+Jira discussion (read the comments), a `prior-tests` basis (`FRESH`/`RE-TEST` — re-tests build on old runs), Exa research, and the **adversarial break-it pass** (step 4.5 — attacks dispositioned, or `no attacks landed` after a real pass). Makes the front-loaded steps non-skippable.
...
- `verify-sidecars.sh <manifest.md> <manifest.json> <report.md> <report.json>` — **md↔json consistency gate**: the markdown gates (context/report/coverage) and the JSON evidence gate each read a *different* file, so this cross-checks that the item IDs match across the two accountings — neither half can pass on a picture the other doesn't share. Structural (same IDs), not semantic.
...
- **Treats its own LLM verifiers as the weakest link** ([verifier-reliability.md](skills/test-iteration/references/verifier-reliability.md)): the review (6.5), re-execution (8.5), and adversarial (4.5) subagents are **probe-gated on a known-correct control** (catch a planted gap / reproduce a known outcome before their verdict counts), **disagreement is surfaced instead of averaged**, and a claimed break is **discarded unless a reproducing test actually goes red** — guarding against LLM silent omission, non-determinism, and over-correction.
