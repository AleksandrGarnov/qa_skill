...
- **API-first — Tinker/REPL only when the API can't prove it.** Anything reachable through the API is exercised **and** observed through the API (script the call as a client); reach for `tinker`/`repl`/`cli` (or the browser/UI) **only when the behaviour genuinely cannot be triggered or verified via the API** — and never to reconstruct the whole flow. If an API call can prove the claim, a REPL read doesn't substitute for it. This is a specialization of "trigger via the real entry point": the API *is* the preferred real entry point; the REPL is a fallback of last resort, not a convenience.
...
- **Know the expected answer independently — don't let the code be its own oracle.** An observation only tests something if you can say what the result *should* be from a source that isn't the implementation. If "expected" is just "whatever the code/dev returned", a green check confirms the code agrees with itself — the same trap as a mock, one level up. For any **computed / money / aggregated / state-transition** AC, derive the expected value **independently and before the run** (a hand calc, the spec's formula, a known invariant, a reference implementation, a trusted historical figure) and record its source; the pass is "raw observation == the independently-derived expected", not "a number came back and it looks plausible". When the correct output genuinely can't be pre-computed (search relevance, ML output, a complex transform, non-deterministic order), fall back to a **metamorphic / property check** — assert an invariant or an input→output relation that must hold (sum of parts == whole, idempotent replay, reversing the op restores the input, reordering inputs changes output predictably) instead of an exact value. See [test-oracle.md](references/test-oracle.md).
...
- **The verifier is the weakest link — verify it too.** The independent subagents this skill relies on (6.5 review, 8.5 re-execution, 4.5 adversarial) are LLMs with known failure modes: silent omission (misses ~6–7× more than it over-flags), run-to-run non-determinism (a lone verdict is often noise), and over-correction (a "break" or "bug" it invented). So **gate the verifier on a known-correct probe** (it must catch a planted gap / reproduce a known outcome before its verdict counts), **surface disagreement instead of averaging it**, and **filter claimed breaks through an actual red test run**. A second model looking at it is corroboration, not proof — only a runnable black-box check is proof. See [verifier-reliability.md](references/verifier-reliability.md).
...
**Filter over-reporting through execution** ([verifier-reliability.md](references/verifier-reliability.md)): an adversarial prompt over-reports — a "break" the subagent *asserts* but **cannot reproduce with a test that actually goes red is a hallucination; discard it**, don't file it. Only a break backed by a red reproducing run is a finding. This is why every landed attack must carry a real reproducing test — the execution is the filter against the model inventing failures.
...
**Probe the reviewer (silent omission is 6–7× harder to catch than over-inclusion — [verifier-reliability.md](references/verifier-reliability.md)).** Hand the reviewer a checklist with **one planted, known gap** (an AC you deliberately left uncovered, or a changed symbol with no item). If the review **doesn't surface the planted gap**, it isn't sensitive enough to trust on the real ones — strengthen the prompt and re-run before believing "nothing missing". Record that the probe was caught.
...
**Probe-gate the blind run, and don't average disagreement** ([verifier-reliability.md](references/verifier-reliability.md)). The re-run is itself an LLM and is non-deterministic — a lone verdict can be noise. So: (1) include **one control with a known outcome** among the re-run commands (a known-good that must come back green, or a deliberately-broken input that must come back red); if the blind run **can't reproduce the known result**, its harness is untrustworthy and its `agree` on the real items doesn't count — fix the harness and re-run. (2) When the blind run **disagrees** with the executor on a borderline critical item, **don't pick one or average** — take one more independent sample and go with the majority, and **record the disagreement rate**, not just the final verdict. Persist `probe: known-good ✓ / known-bad ✓` and any `disagreement` in `report.json` so "the verifier was itself verified" is visible.
